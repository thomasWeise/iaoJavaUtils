...
/** This class provides some utilities for I/O */
public final class IOUtils {

  /**
   * Store a node to a binary file
   *
   * @param dir
   *          the directory
   * @param suffix
   ...
   *           on io error
   */
  public static final Path uniqueFileInDir(final Path dir,
      final String suffix) throws IOException {
    final String usePrefix = "class_"; //$NON-NLS-1$
    final int maxIndex = 10_000_000;
    int index;

    index = 1;
    for (; index < 10; index++) {
      final Path file = dir.resolve((((usePrefix + index) + suffix)));
      try {
        return Files.createFile(file);
      } catch (@SuppressWarnings("unused") final IOException t) {
        //
      }
    }
    ...
    while (index <= high) {
      final int mid = (index + high) >>> 1;
      if (Files.exists(dir.resolve(//
          (((usePrefix + mid) + suffix))))) {
        index = mid + 1;
      } else {
        high = mid - 1;
      }
    ...
    return Files.createTempFile(usePrefix, suffix);
  }
  ...
  /**
   * Remove the file extension from the given name, if any
   *
   * @param name
   *          the file name
   * @return the file name without extension
   */
  public static final String removeFileExtension(final String name) {
